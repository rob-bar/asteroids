...
$(document).ready(function() {
	...
	function checkKeys() {
		if (key_press_list[38] == true) {
			...
			var movingXnew =
				player.movingX + player.thrustAcceleration * player.facingX;
			var movingYnew =
				player.movingY + player.thrustAcceleration * player.facingY;

			var curentVelocity = Math.sqrt(
	...
	function gameStateNewGame() {
		...
		player.scale = 1;
		player.alpha = 0;
		player.halfWidth = player.width * 0.5;
		player.halfHeight = player.height * 0.5;
	...
	function gameStateNewLevel() {
		...
		for (i = 0; i < level.level + 3; i++) {
			...
			newRock.dx = Math.floor(
				Math.random() * 2 + level.level_rock_max_speed_adjust
			);
			if (Math.random() < 0.5) newRock.dx = -1;

			newRock.dy = Math.floor(
				Math.random() * 2 + level.level_rock_max_speed_adjust
			);
	...
	function gameStateGameOver() {
		...
		if (game_over_started == false) {
			...
			context.fillText('Game Over!', x_max * 0.5, 70);
			context.fillText('Press Space To Play', x_max * 0.5, 140);
	...
	function renderScoreboard() {
		...
		playerIndic.x = x_max * 0.5 - 32;
		playerIndic.y = 16;
		playerIndic.width = 20;
		playerIndic.height = 20;
		playerIndic.halfWidth = playerIndic.width * 0.5;
		playerIndic.halfHeight = playerIndic.height * 0.5;
		playerIndic.rotation = 270;
		playerIndic.scale = 0.75;
		playerIndic.render(context);
		context.textAlign = 'center';
		context.fillText('X ' + player_ships, x_max * 0.5, 20);
	...
	function updateSaucers() {
		...
		if (saucers.length < level.level_saucer_max) {
			if (
				Math.floor(Math.random() * 100) <= level.level_saucer_occurence_rate
			) {
				var newSaucer = new Saucer(
					0,
					0,
					0,
					0,
					...
					13,
					saucer_rock_score,
					level.level_saucer_fire_rate,
					level.level_saucer_fire_delay,
					0,
					level.level_saucer_missile_speed
				);
	...
	function checkCollisions() {
		...
		rocksing: for (var rockCtr = rocksLength; rockCtr >= 0; rockCtr--) {
			tempRock = rocks[rockCtr];
			missilesing: for (
				var playerMissileCtr = playerMissileLength;
				playerMissileCtr >= 0;
				playerMissileCtr--
			) {
			...
			saucering: for (
				...
				if (boundingBoxCollide(tempRock, tempSaucer)) {
					...
					createExplode(
						tempSaucer.x + tempSaucer.halfWidth,
						tempSaucer.y + tempSaucer.halfHeight,
						10
					);
					createExplode(
						tempRock.x + tempRock.halfWidth,
		...
		saucersying: for (saucerCtr = saucerLength; saucerCtr >= 0; saucerCtr--) {
			tempSaucer = saucers[saucerCtr];
			missilesying: for (
				playerMissileCtr = playerMissilesLength;
				playerMissileCtr >= 0;
				playerMissileCtr--
			) {
				tempPlayerMissile = player.aPlayerMissiles[playerMissileCtr];
			...
			saucerMissilesYeah: for (
				saucerMissileCtr = saucerMissileLength;
				saucerMissileCtr >= 0;
				saucerMissileCtr--
			) {
				tempSaucerMissile = tempSaucer.aSoucerMissiles[saucerMissileCtr];
	...
	function createExplode(x, y, num) {
		...
		for (var partCtr = 0; partCtr < num; partCtr++) {
			...
			if (Math.random() < 0.5) particle.dx *= -1;

			particle.dy = Math.random() * 3;
			if (Math.random() < 0.5) particle.dy *= -1;
	...
	function splitRock(scale, x, y) {
		...
		for (rockCtr = 0; rockCtr < 2; rockCtr++) {
			...
			if (Math.random() < 0.5) rock.dx *= -1;

			rock.dy = Math.random() * 3;
			if (Math.random() < 0.5) rock.dy *= -1;

			rock.rotationInc = Math.random() * 5 + 1;
			if (Math.random() < 0.5) rock.rotationInc *= -1;
